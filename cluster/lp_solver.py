...
#     以「最小化最大链路利用率 (MLU)」为目标生成分流比标签，并应用标签平滑处理。
#     核心变更：
#     1. 优化目标改为「最小化全局 MLU 变量 U」。
#     2. 移除需求满足比例变量 d[i]，强制所有需求必须被 100% 满足（分流比和为 1）。
#     3. 约束调整为「每条链路的总负载 ≤ U * 链路容量」。
#     """
#     print(f"正在从 {input_pkl} 加载数据集...")
#     with open(input_pkl, 'rb') as f:
#         dataset = pickle.load(f)
...
#
#         # 构建有向链路索引
#         link_list = [tuple(e) for e in edges]
...
#         # 处理求解结果（生成分流比标签）
#         if m.status == GRB.OPTIMAL:
#             print(f"  时间片 {ts_idx}: 求解成功，最优 MLU = {U.X:.4f} (对应最大负载: {U.X * capacity:.2f})")
#
#             ratios_np = np.zeros((num_demands, k), dtype=np.float64)
#             for i in range(num_demands):
#                 for j in range(k):
...
#
#         else:
#             print(f"  警告：时间片 {ts_idx} 未能找到最优解 (状态码: {m.status})，可能由于网络物理断连导致无法承载所有流量。填充均匀分布。")
#             current_ts_tensor = torch.full((num_demands, k), 1.0 / k, dtype=torch.float32)
#
#         final_label_list.append(current_ts_tensor)
#         print(f"  时间片 {ts_idx} 处理耗时: {time.time() - ts_start_time:.2f} 秒。")
...
#
# if __name__ == "__main__":
#     input_data_pkl = "F:\Py_Project\\always\cluster\zone\outputs\\tms\starlink550_cluster.pkl"
#     output_label_pkl = "F:\Py_Project\\always\cluster\zone\outputs\\tms\Labels_starlink550_cluster.pkl"
#
#     # 按照需求，容量与平滑参数保持一致
#     labels = generate_labels_min_mlu(input_data_pkl, output_label_pkl, capacity=250, smoothing=0.1)